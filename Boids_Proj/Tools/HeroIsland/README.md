...
- 垂藤采用更小、更薄的叶片与较多分枝；叶片保留局部 UV，近景可见淡叶脉，远景按像素覆盖率淡出，减少闪烁。
...
`SkyCityHeroCloudBuilder` 为主岛配置低处连续云海、中景积云和高处薄云。Compute Shader 将造型与三维细胞噪声合成为 256×128×256 的密度场，并预计算太阳方向上的遮光。运行时读取这个 32 MiB 的 RGBA32 数据场，云内采用 0.32 米积分步距，默认上限 1536 步覆盖整个可视范围，避免固定低采样数造成阴影条纹或截掉远景；空区域跳过、密集云体提前结束，平面反射使用较低分辨率。两个颜色通道共同保存遮光值，保留低光学深度区域的精度。

云的位置缓慢漂移，内部细节随风变化；高低云均为三维密度，不使用云朵图片或背景投影。主相机使用完整渲染分辨率。`Capture(name, cloudTime)` 可固定云的时间，便于同一构图下比较修改；正常播放使用实时钟。

主岛当前采用固定日照方向，因此云内遮光可以缓存。修改太阳方向、云密度或体积构图后，要在 Edit 模式执行 `Boids > Hanging Gardens > Sculpt cloud sea`；完整建场景会自动执行。此缓存专用于该主岛样板，不代表无限世界已经采用相同实现。

技术阅读参考：[Guerrilla 的 Nubis 体积云资料](https://www.guerrilla-games.com/read/nubis-authoring-real-time-volumetric-cloudscapes-with-the-decima-engine)。本项目的固定场景缓存与造型代码为本工程实现。
...
Windows 独立程序，RTX 3090，1536×1024，60 fps 上限：40.006 秒内实际渲染 2400 帧，P95 16.670 ms、P99 16.729 ms，最大 17.308 ms；GPU P95 7.463 ms。反射更新 972 次，鸟群运动检查通过，运行错误 0。构建错误与警告均为 0。该结果只适用于此样板与上述硬件、分辨率。

三级建筑 LOD 分别为 1,975,761、754,164、261,287 三角形；另有少量独立的水面、瀑布和旗帜。场景包含 2 张光照图及 325 个光照探针。环绕、平移、缩放及 F 返回初始位置已在 Play 模式验证。
