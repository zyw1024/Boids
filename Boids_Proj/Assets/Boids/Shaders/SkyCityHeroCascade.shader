Shader "Boids/SkyCity/Hanging Cascades"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(V i):SV_Target
            {
                ...
                float2 flow=float2(i.uv.x*13,(i.uv.y+i.uv.y*i.uv.y*.9)*16-t*6);
                ...
                float fade=1-smoothstep(.78,1,i.uv.y);
                ...
                float3 c=lerp(behind*float3(.66,.90,.96),float3(1.10,1.12,1.06),saturate(.34+foam*.74+fresnel*.20));
                c+=sun.color*spec*.7;
                float alpha=(.52+strands*.35+spray*.13)*edge*fade*saturate(gap*3);
