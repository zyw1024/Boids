...
namespace SkyCity.Runtime
{
    public sealed class SkyCityGardenDirector : MonoBehaviour
    {
        ...
        readonly HashSet<SkyCityLivingGarden> streamedGardens=new HashSet<SkyCityLivingGarden>();
        public IEnumerable<SkyCityLivingGarden> AllGardens
        {get{if(gardens!=null)foreach(var g in gardens)if(g!=null)yield return g;foreach(var g in streamedGardens)if(g!=null)yield return g;}}
        public void Register(SkyCityLivingGarden g){streamedGardens.Add(g);}
        public void Unregister(SkyCityLivingGarden g){streamedGardens.Remove(g);}
        Renderer[] treeRenderers;
        MaterialPropertyBlock treeProperties;
        float noticeUntil;
        string notice;
        public int Blooms {get{int n=0;foreach(var g in AllGardens)if(g.isActiveAndEnabled&&g.Simulating&&g.Simulation!=null)n+=g.Simulation.BloomCount;return n;}}
        void Awake()
        {
        ...
        {
            foreach(var g in AllGardens)
            {
                if(!g.isActiveAndEnabled||player==null||(g.transform.position-player.view.transform.position).sqrMagnitude>10000)continue;
                for(float u=g.bedSpacing*.5f;u<g.size.x;u+=g.bedSpacing)g.Sow(Mathf.Clamp((int)(u/g.size.x*g.columns),0,g.columns-1),g.rows/2);
                if(player.flock!=null)player.flock.VisitGarden(g.transform.position);
            }
        }
        ...
        public void RefreshAppearance(){RefreshTree();foreach(var g in AllGardens)g.RefreshAppearance();}
        public void ClearGardens(){windSeeds=false;foreach(var g in AllGardens)g.ResetGarden();}
