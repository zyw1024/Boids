...
public static class SkyCityStyleReview
{
    public static void RefreshAuthoredMeshes(bool rebuildLightingMeshes=false)
    {
        ...
        if(scene!=SkyCityWorldBuilder.ScenePath)throw new InvalidOperationException("Open SkyCityWorld.");
        var models=new[]{"HangingGardens","HangingGardens_LOD1","HangingGardens_LOD2"}
            .SelectMany(n=>AssetDatabase.LoadAssetAtPath<GameObject>(SkyCityAssetPaths.Island+"/"+n+".fbx").GetComponentsInChildren<MeshFilter>()).ToDictionary(f=>f.name);
        if(rebuildLightingMeshes)
        {
            foreach(string prefix in new[]{"01","06"})
            {
                ...
                string path=SkyCityAssetPaths.Island+"/LightingMeshes/"+prefix+".asset";var saved=AssetDatabase.LoadAssetAtPath<Mesh>(path);
        ...
        foreach(var filter in root.GetComponentsInChildren<MeshFilter>(true))
        {
            ...
            filter.sharedMesh=!filter.name.Contains("distance")&&(prefix=="01"||prefix=="06")?AssetDatabase.LoadAssetAtPath<Mesh>(SkyCityAssetPaths.Island+"/LightingMeshes/"+prefix+".asset"):source.sharedMesh;
            if(prefix=="06")
            {
    ...
    public static void ApplyReferenceStyle()
    {
        ...
        var stone=Load(SkyCityAssetPaths.Island,"Warm carved limestone");
        var copper=Load(SkyCityAssetPaths.Island,"Quiet verdigris copper");
        var water=Load(SkyCityAssetPaths.Island,"Sheltered reflecting water");
        string normalPath=SkyCityAssetPaths.Island+"/LimestoneScanNormal.jpg";
        ...
        var albedo=AssetDatabase.LoadAssetAtPath<Texture2D>(SkyCityAssetPaths.Shared+"/Textures/WeatheredLimestone.png");
        var normal=AssetDatabase.LoadAssetAtPath<Texture2D>(normalPath);
        var cliff=Load(SkyCityAssetPaths.Island,"Stratified pale cliff");
        ...
        foreach(var material in new[]{Load(SkyCityModuleBuilder.Root,"Ivory copper and silk"),Load(SkyCityWorldBuilder.Root,"Weathered ivory city")})
        {
        ...
        foreach(var material in new[]{Load(SkyCityModuleBuilder.Root,"Infinite reflecting water"),Load(SkyCityWorldBuilder.Root,"Archipelago reflecting pools")})
        {
    ...
    {
        var shaders=AssetDatabase.FindAssets("t:Shader",new[]{"Assets/SkyCity/Shaders"})
            .Select(id=>AssetDatabase.LoadAssetAtPath<Shader>(AssetDatabase.GUIDToAssetPath(id)))
            .Where(s=>s.name.StartsWith("SkyCity/"));
        ...
        var stone=Load(SkyCityAssetPaths.Island,"Warm carved limestone");
        var materials=new[]{Load(SkyCityModuleBuilder.Root,"Ivory copper and silk"),Load(SkyCityWorldBuilder.Root,"Weathered ivory city")};
        ...
        var report=new{passed=errors.Length==0&&match,shaderErrors=errors,stoneParametersMatchReference=match,referenceScene=SkyCityWorldBuilder.ScenePath,
