...
namespace Boids.Art
{
    ...
    public sealed class SkyCityLivingGarden : MonoBehaviour
    {
        ...
        {
            public byte[] cells;public float[] ages;
            public int generation,blooms,awakened,windSequence,seedsPlanted;
            public float windTime,accumulator;
        }
        ...
        public void Initialize()
        {
            ...
            if(director==null)director=GetComponentInParent<SkyCityGardenDirector>();
        ...
        public State CaptureState()
        {
            ...
            var cells=new byte[Simulation.Cells.Length];for(int i=0;i<cells.Length;i++)cells[i]=(byte)Simulation.Cells[i];
            return new State{cells=cells,ages=(float[])Simulation.Ages.Clone(),generation=Simulation.Generation,blooms=Simulation.BloomCount,awakened=Simulation.Awakened,windTime=windTime,windSequence=windSequence,accumulator=accumulator,seedsPlanted=SeedsPlanted};
        }
        public void RestoreState(State state)
        {
            Initialize();if(state==null)return;
            Simulation.Restore(state.cells,state.ages,state.generation,state.blooms,state.awakened);
            windTime=state.windTime;windSequence=state.windSequence;accumulator=state.accumulator;SeedsPlanted=state.seedsPlanted;Publish(true);
        }
